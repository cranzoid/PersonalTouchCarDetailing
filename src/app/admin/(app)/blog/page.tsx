  ...
  return (
    <div className="max-w-6xl">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div><p className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#8A681F]">SEO content</p><h1 className="mt-1 text-2xl font-bold text-[#0B2A4A]">Blog</h1><p className="mt-1 max-w-2xl text-sm text-[#5A6B7D]">Write, review and publish useful articles for local drivers. Drafts stay private until you publish them.</p></div>
        <Link href="/admin/blog/new" className="inline-flex min-h-11 items-center rounded-xl bg-[#0B2A4A] px-4 text-sm font-bold text-white admin-on-dark shadow-sm">New article</Link>
      </header>
      <section className="mt-6 rounded-2xl border border-[#DDE4EC] bg-white p-5 shadow-sm">
        {posts.length === 0 ? (
          <div className="rounded-xl bg-[#F5F7FA] px-5 py-12 text-center"><h2 className="font-bold text-[#0B2A4A]">No articles yet</h2><p className="mt-2 text-sm text-[#5A6B7D]">Create your first draft, then publish it when the copy and search preview are ready.</p></div>
        ) : (
